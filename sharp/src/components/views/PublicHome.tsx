      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <BrutalCard color={COLORS.yellow} className="relative min-h-[300px] flex flex-col justify-end p-6 group">
          ...
          <div className="space-y-3 z-10">
            ...
            <BrutalButton color="white" className="w-fit rounded-full px-5 py-2.5 text-[9px]" onClick={() => setActiveTab('explore')}>
        ...
        <BrutalCard color={COLORS.teal} className="relative min-h-[300px] flex flex-col items-center justify-center text-center p-6">
          ...
          <div className="mt-5 flex items-center gap-2">
            ...
            <span className="font-black text-[9px]">{events.length} EVENTS LIVE</span>
          </div>
        </BrutalCard>
      </div>

      {/* Top Picks — Real Events */}
      <section>
        <div className="flex justify-between items-end mb-6">
          ...
          <BrutalButton color="white" className="text-[8px] px-3 py-1.5" onClick={() => setActiveTab('explore')}>Explore More</BrutalButton>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {featured.length === 0 ? (
            <BrutalCard className="col-span-4 p-6 text-center">
              <p className="text-[11px] font-black uppercase italic opacity-30">No events yet — check back soon!</p>
            </BrutalCard>
          ) : (
            featured.map((evt, i) => {
